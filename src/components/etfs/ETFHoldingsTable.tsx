    ...
    <div className="space-y-6">
          ...
          {[
            ...
            <div key={s.label} className="rounded-2xl p-5 flex flex-col gap-3" style={CARD_STYLE}>
              ...
              <div>
                ...
                {s.sub && <p className="text-xs mt-0.5" style={LABEL_COLOR}>{s.sub}</p>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={CARD_STYLE}>
        <div className="px-6 py-4 flex items-center justify-between"
          ...
          <div>
            <h3 className="font-semibold" style={TEXT_COLOR}>Positions</h3>
            {holdings.length > 0 && (
              <p className="text-xs mt-0.5" style={LABEL_COLOR}>{holdings.length} position{holdings.length !== 1 ? 's' : ''}</p>
            )}
          </div>
        ...
        {holdings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="font-semibold mb-1" style={TEXT_COLOR}>No ETF positions yet</p>
            <p className="text-sm mb-4" style={LABEL_COLOR}>Track your ASX and US ETF investments</p>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {holdings.map((h) => {
                  ...
                  return (
                    <tr key={h.id} style={{ borderBottom: '1px solid rgba(99,102,241,0.08)' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(99,102,241,0.05)' }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '' }}>
                      <td className="px-6 py-4">
                        <p className="font-bold text-base" style={TEXT_COLOR}>{h.ticker}</p>
                        <p className="text-[11px] mt-0.5" style={LABEL_COLOR}>{h.name}</p>
                        {h.sector && (
                      ...
                      <td className="text-right px-4 py-4 font-mono text-xs" style={LABEL_COLOR}>{h.shares.toFixed(4)}</td>
                      <td className="text-right px-4 py-4 text-xs" style={LABEL_COLOR}>
                        {h.currency === 'AUD' ? 'A$' : '$'}{h.avg_purchase_price?.toFixed(2) ?? '—'}
                      </td>
                      <td className="text-right px-4 py-4 font-semibold" style={TEXT_COLOR}>
                        {h.currency === 'AUD' ? 'A$' : '$'}{h.current_price?.toFixed(2) ?? '—'}
                      </td>
                      <td className="text-right px-4 py-4 font-semibold" style={TEXT_COLOR}>{formatAUD(currentValue)}</td>
