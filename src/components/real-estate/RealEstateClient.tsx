...
import {
  ...
  ExternalLink, ChevronDown, ChevronUp, Edit2, CalendarDays,
} from 'lucide-react'
...
const DEDUCT_TYPES = ['Fully deductible','Depreciation (non-cash)','CGT cost base','Not deductible','Rental income']
...
// ── Cash flow calculator (mirrors FinTrack logic) ─────────────────────────────
interface CFResult {
  ...
  paymentCount: number
  grossRent: number
  estimatedRent: number      // auto-calc from weekly_rent
  ...
  netRental: number          // net position (income − deductions incl depreciation)
  taxSaving: number          // negative gearing benefit
  cashFlow: number           // actual cash (excl. depreciation) + tax saving
  perWeek: number
  grossYield: number
  loanBalance: number        // estimated loan balance (P&I amortisation)
...
function calcCF(
  ...
  // Interest from amortised balance (use loanBalance for current FY)
  ...
  const rentStart    = tenancyDate
    ? (tenancyDate >= fyStart ? tenancyDate : fyStart)
    : null
  let rentPayments = 0
  if (rentStart) {
  ...
  // Pro-rate annual costs by active days in FY
  ...
  // Use actual logged expenses where available (actual wins over estimate)
  ...
  // Cash flow (real money — depreciation excluded from cash but included in tax calc)
...
export function RealEstateClient({ properties, expenses: initExp, income: initInc, userId }: Props) {
  ...
  const [editPropId,   setEditPropId]   = useState<string | null>(null)  // inline edit
  ...
  // ── Forms ──────────────────────────────────────────────────────────────────
  const [propF, setPropF] = useState({ ...PROP_DEFAULTS })
  const [editF, setEditF] = useState({ ...PROP_DEFAULTS })
  ...
  const fyDeductible = filteredExp.filter(e => {
    ...
    return atoCat?.deduct === 'Fully deductible' || atoCat?.deduct === 'Depreciation (non-cash)'
  ...
  // ── Property form → DB object ─────────────────────────────────────────────
  function formToInsert(f: typeof PROP_DEFAULTS, uid: string) {
  ...
  // ── Populate edit form from existing property ─────────────────────────────
  ...
  async function saveNewProperty() {
    ...
    const { error } = await supabase.from('real_estate').insert(formToInsert(propF, userId))
    setSaving(false)
  ...
  async function saveEditProperty(id: string) {
    setSaving(true); setError('')
    const supabase = createClient()
    const payload  = formToInsert(editF, userId)
    const { error } = await supabase.from('real_estate').update(payload).eq('id', id)
    setSaving(false)
    if (error) { setError(error.message); return }
    setEditPropId(null); router.refresh()
  }
  ...
  async function deleteExpense(id: string) { await createClient().from('real_estate_expenses').delete().eq('id', id); setExpenses(prev => prev.filter(e => e.id !== id)) }
  async function deleteIncome(id: string)  { await createClient().from('real_estate_income').delete().eq('id', id);  setIncome(prev => prev.filter(i => i.id !== id)) }
  ...
  function exportCSV() {
    const rows = [
      ...
      [],
      ['EXPENSES'],
      ['Date', 'Property', 'Category', 'Description', 'Deductibility', 'Amount'],
      ...filteredExp.map(e => {
        const atoCat = ATO_CATS.find(c => c.cat === e.category)
        return [e.date, propName(e.property_id), e.category, e.description ?? '', atoCat?.deduct ?? '', e.amount.toFixed(2)]
      }),
      ['', '', '', '', 'TOTAL EXPENSES', fyExpenses.toFixed(2)],
      ['', '', '', '', 'DEDUCTIBLE', fyDeductible.toFixed(2)],
      [],
      ['SUMMARY'],
      ['Net Rental Position', (fyIncome - fyExpenses).toFixed(2)],
    ]
  ...
  function PropertyForm({ f, up, title, subtitle, onSave, onCancel }: {
    f: typeof PROP_DEFAULTS
    ...
    const row = (label: string, key: string, type = 'text', placeholder = '', opts?: string[]) => (
      <div>
        ...
        {opts ? (
          ...
          <input type={type} value={(f as Record<string,string>)[key]} onChange={e => up(key, e.target.value)}
            placeholder={placeholder} className="w-full h-9 px-3 rounded-lg text-sm input-dark" />
        )}
      </div>
    )
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
        <div className="w-full max-w-2xl rounded-2xl max-h-[92vh] flex flex-col" style={MODAL}>
          <div className="px-6 py-5 flex items-center justify-between shrink-0" style={{ borderBottom: '1px solid rgba(99,102,241,0.2)' }}>
            <div><h2 className="font-bold text-lg" style={TC}>{title}</h2><p className="text-xs mt-0.5" style={LC}>{subtitle}</p></div>
            <button onClick={onCancel} className="h-8 w-8 rounded-lg flex items-center justify-center" style={LC}><X className="h-4 w-4" /></button>
          </div>
          <div className="p-6 overflow-y-auto flex-1">
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              {section('Property Details')}
              <div className="col-span-full">{row('Property Name *', 'name', 'text', '15 Dale Crescent, Armidale NSW 2350')}</div>
              ...
              {section('Annual Operating Costs (for auto cash flow)')}
              ...
              {section('Ownership & Tax (for negative gearing)')}
            ...
            {error && <div className="mt-4 text-xs px-3 py-2 rounded-lg" style={{ background: 'rgba(239,68,68,0.1)', color: '#fca5a5' }}>{error}</div>}
          </div>
  ...
  function CashFlowTable({ p }: { p: RealEstate }) {
      ...
      return (
        <div className="rounded-xl p-4 mt-4" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-amber-400">Settlement {days > 0 ? `in ${days} days` : 'today!'}</p>
              <p className="text-xs mt-0.5" style={LC}>{p.settlement_date} — Cash flow activates after settlement</p>
            </div>
            {days > 0 && <div className="ml-auto text-right">
    ...
    const rows = [
      { label: cf.isPreTenancy ? 'Gross Rental Income' : 'Gross Rental Income', val: cf.grossRent, green: true, note: cf.isPreTenancy ? 'pre-tenancy (no rent yet)' : cf.estimatedRent !== cf.grossRent ? 'from logged income' : `${Math.round(cf.grossRent / (p.weekly_rent ?? 1))} weeks` },
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} style={{ borderBottom: '1px solid rgba(99,102,241,0.07)' }}>
                  <td className="px-3 py-2" style={LC}>
                    {r.label}{r.note && <span className="ml-1 text-[10px]" style={{ color: 'rgba(161,174,255,0.3)' }}>({r.note})</span>}
                  </td>
              ...
              {cf.taxSaving > 0 && (
                <tr style={{ borderBottom: '1px solid rgba(99,102,241,0.07)' }}>
                  <td className="px-3 py-2 text-emerald-400" style={{ fontSize: '11px' }}>Tax Saving (negative gearing)</td>
                  <td className="px-3 py-2 text-right font-semibold text-emerald-400 tabular-nums" style={{ fontSize: '11px' }}>+${cf.taxSaving.toLocaleString('en-AU', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</td>
                </tr>
              )}
              {/* After-tax */}
              <tr style={{ background: cf.cashFlow >= 0 ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)' }}>
                <td className="px-3 py-2.5 font-bold text-xs" style={{ color: cf.cashFlow >= 0 ? '#6ee7b7' : '#fca5a5' }}>
                  After-Tax Cash Flow
                  <span className="ml-2 font-normal text-[10px]" style={LC}>{cf.perWeek >= 0 ? '+' : ''}${Math.abs(cf.perWeek).toFixed(0)}/wk</span>
                </td>
    ...
    <div className="space-y-5">
      ...
      {tab === 'properties' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <FilterBar />
            <button onClick={() => { setError(''); setShowProp(true) }} className="btn-gradient flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-white">
          ...
          {properties.length === 0 ? (
            <div className="rounded-2xl flex flex-col items-center justify-center py-20 text-center" style={CARD}>
              ...
              <button onClick={() => setShowProp(true)} className="btn-gradient px-4 py-2 rounded-lg text-sm text-white">Add your first property →</button>
            </div>
          ) : properties.map(p => {
            const cf       = calcCF(p, expenses, income, selectedFY)
            const isEditing = editPropId === p.id

            const statusColor = p.property_status === 'Tenanted' ? { bg: 'rgba(16,185,129,0.12)', text: '#6ee7b7' }
              : p.property_status === 'Pre-tenancy' ? { bg: 'rgba(245,158,11,0.12)', text: '#fcd34d' }
            ...
            return (
              <div key={p.id} className="rounded-2xl overflow-hidden" style={CARD}>
                ...
                <div className="px-5 py-4 flex items-start gap-4" style={{ borderBottom: '1px solid rgba(99,102,241,0.1)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {p.lender && <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(99,102,241,0.1)', color: '#818cf8' }}>{p.lender}</span>}
                    </div>
                    {p.address && <p className="text-xs mt-0.5 truncate" style={LC}>{p.address}</p>}
                    {p.loan_type && <p className="text-[10px] mt-0.5" style={{ color: 'rgba(161,174,255,0.35)' }}>{p.loan_type} · {p.rate_type} · {p.loan_rate}% · {p.loan_term_years}yr</p>}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => isEditing ? setEditPropId(null) : openEdit(p)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all"
                      style={isEditing ? { background: 'rgba(239,68,68,0.12)', color: '#fca5a5', border: '1px solid rgba(239,68,68,0.2)' } : { background: 'rgba(99,102,241,0.1)', color: '#a5b4fc', border: '1px solid rgba(99,102,241,0.2)' }}>
                ...
                {isEditing && (
                  <div className="p-5" style={{ borderBottom: '1px solid rgba(99,102,241,0.1)', background: 'rgba(99,102,241,0.03)' }}>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {/* Quick-edit most important fields */}
                      {([
                        ...
                        <div key={key}>
                          ...
                          <input type={type} value={(editF as Record<string,string>)[key]} onChange={e => upEdit(key, e.target.value)}
                            className="w-full h-8 px-2.5 rounded-lg text-xs input-dark" />
                        </div>
                      ))}
                    </div>
                    <div className="flex items-center gap-2 mt-3">
                      <button onClick={() => saveEditProperty(p.id)} disabled={saving}
                      ...
                      <button onClick={() => { setError(''); setEditF({ ...editF }); setShowProp(true); /* TODO: full edit modal */ }}
                        className="btn-ghost px-3 py-1.5 rounded-lg text-xs" style={LC}>Full Edit →</button>
                ...
                <div className="p-5">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                    {[
                      ...
                      <div key={m.label} className="rounded-xl p-3"
                        style={{ background: m.green ? 'rgba(16,185,129,0.07)' : m.red ? 'rgba(239,68,68,0.07)' : 'rgba(99,102,241,0.06)', border: `1px solid ${m.green ? 'rgba(16,185,129,0.12)' : m.red ? 'rgba(239,68,68,0.1)' : 'rgba(99,102,241,0.1)'}` }}>
                        <p className="text-[10px] font-semibold uppercase tracking-wider mb-1" style={LC}>{m.label}</p>
                        <p className={`font-bold text-sm ${m.green ? 'text-emerald-400' : m.red ? 'text-red-400' : ''}`} style={m.green || m.red ? {} : TC}>{m.val}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* FY cash flow table */}
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(161,174,255,0.5)' }}>{fyLabel(selectedFY)} Cash Flow</p>
                    <CashFlowTable p={p} />
                  </div>
                </div>
              </div>
      ...
      {tab === 'income' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <FilterBar />
            <button onClick={() => { setError(''); setShowInc(true) }} className="btn-gradient flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-white" disabled={properties.length === 0}>
          ...
          <div className="rounded-2xl overflow-hidden" style={CARD}>
            {filteredInc.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead><tr style={{ borderBottom: '1px solid rgba(99,102,241,0.12)', background: 'rgba(99,102,241,0.04)' }}>
                  {['Date','Property','Description','Amount',''].map(h => <th key={h} className={`px-5 py-3 text-[10px] font-semibold uppercase tracking-widest ${h === 'Amount' ? 'text-right' : 'text-left'}`} style={LC}>{h}</th>)}
                </tr></thead>
                <tbody>{filteredInc.map(i => (
                  <tr key={i.id} className="group transition-colors" style={{ borderBottom: '1px solid rgba(99,102,241,0.07)' }}
                    ...
                    <td className="px-3 py-3"><button onClick={() => deleteIncome(i.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded" style={{ color: 'rgba(239,68,68,0.6)' }}><Trash2 className="h-3.5 w-3.5" /></button></td>
                  </tr>
                ))}</tbody>
              </table>
            )}
      ...
      {tab === 'expenses' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <FilterBar />
            <button onClick={() => { setError(''); setShowExp(true) }} className="btn-gradient flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-white" disabled={properties.length === 0}>
            ...
            {filteredExp.length === 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>{filteredExp.map(e => {
                  ...
                  return (
                    <tr key={e.id} className="group transition-colors" style={{ borderBottom: '1px solid rgba(99,102,241,0.07)' }}
                      ...
                      <td className="px-4 py-3 text-[11px]" style={{ color: isDeductible ? '#6ee7b7' : '#fcd34d' }}>
                        {atoCat?.deduct ?? '—'}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-red-400 tabular-nums text-xs">{formatAUD(e.amount)}</td>
                      <td className="px-3 py-3"><button onClick={() => deleteExpense(e.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded" style={{ color: 'rgba(239,68,68,0.6)' }}><Trash2 className="h-3.5 w-3.5" /></button></td>
                    </tr>
                  )
                })}</tbody>
              </table>
      ...
      {tab === 'summary' && (
        <div className="space-y-4">
            ...
            return (
              <div key={p.id} className="rounded-2xl overflow-hidden" style={CARD}>
                <div className="px-5 py-3" style={{ borderBottom: '1px solid rgba(99,102,241,0.12)' }}>
                  ...
                  <p className="text-xs mt-0.5" style={LC}>Auto-estimated from property settings · logged expenses take precedence</p>
                </div>
                <div className="p-5"><CashFlowTable p={p} /></div>
              </div>
            )
          })}
          ...
          <div className="rounded-xl px-4 py-3 text-xs flex items-start gap-2"
            ...
            <span>This is a guide only, not tax advice. Confirm deductibility with your registered tax agent before lodging. Capital improvements and mixed-use properties require apportionment.</span>
          </div>
        </div>
      )}
      ...
      {showProp && (
        <PropertyForm
          ...
          subtitle="Fill in the key details — the app will auto-calculate cash flow and tax impact"
